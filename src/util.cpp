...
const char * const BITCOIN_CONF_FILENAME = "vikascoin.conf";
const char * const BITCOIN_PID_FILENAME = "vikascoin.pid";
...
#else
    const char* pszModule = "vikascoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Vikascoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Vikascoin";
#else
    // Unix
    return pathRet / ".vikascoin";
#endif
#endif
}
